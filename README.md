...
With all these characters, we can concatenate them into strings that form normal javascript code. And using the function caller, we can actually execute the string, very similar to `eval()`. Replacing all the variable names with non-alphanumeric variables such as $ and _, we can write complete and fully functioning non-alphanumeric code:
```javascript
